...
def get_mongodb():
    params = config.MONGODB.copy()
    name = params.pop('name')
    return pymongo.MongoClient(**params)[name]


class Model(pymongo.collection.Collection):
    _fields = None

    def __init__(self, *args, **kwargs):
        super().__init__(get_mongodb(), *args, **kwargs)

        if self._fields:
            self._fields.update({
                '_id': (bson.ObjectId, False),
    ...
    def _filter_none_value(doc):
        if not isinstance(doc, dict):
            raise error.Error(message='doc should be a dict')
    ...
    def _validate(cls, doc, required=True):
        ...
        for k, v in doc.items():
            ...
            if k not in cls._fields:
                raise error.Error(message='unexpected field {}'.format(k))
        ...
        if required:
            ...
            for v in fields:
                if v not in doc:
                    raise error.Error(message='field {} is required'.format(v))
